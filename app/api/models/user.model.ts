...
import { Table, Column, DataType, BeforeCreate, BeforeSave, ForeignKey } from 'sequelize-typescript';
...
export class User extends BaseModel<User> implements IUser {
    ...
    public static async verificaEmailCadastrado(user: User, options: any) {
        ...
        if (c > 0)
            throw new HttpException('Já existe um usuário utilizando o email informado!');
        return;
    }
    ...
    public setSenha(senhaNova: string) {
        if (senhaNova.length < 8) throw new HttpException('A senha deve possuir no minimo de 8 caracteres')
    ...
    public generateToken(isApi: boolean = false) {
        let token = jwt.sign({ sub: this.id }, process.env.JWT_SECRET + '', { expiresIn: process.env.JWT_EXP_TIME });
    ...
    public generateTokenResetSenha() {
        return jwt.sign({ usr: this.id, res: true }, process.env.JWT_SECRET + '', { expiresIn: '1h' });
    }
